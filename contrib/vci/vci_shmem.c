...
vci_Initialize_devload_info(void)
{
	...
	/**
	 * OSS FIXME  - most of this loop is redundand because in the end we are
	 * we are only using "unmoniored" devices
	 */
	for (i = 0; i < VciShmemAddr->max_devices + 1; i++)
	{
		vci_devload_t *devinfo = &(VciShmemAddr->devload_array[i]);

		/* init */
		strcpy(devinfo->devname, VCI_FREE_DEVLOAD_INFO_NAME);
