...
registerMemoryEntry2Device(vci_memory_entry_t *entry)
{
	...
	dload = vci_find_devloadinfo(VciShmemAddr->devload_array,
								 VciShmemAddr->num_devload_info, devname);
	Assert(dload != NULL);
...
vci_GetWosRosConvertingVCI(vci_wosros_conv_worker_arg_t *vciinfo)
{
		   ...
		   && !found)
	{
		...
		if (dl->memory_entry_pos == NULL
			||
			(strcmp(dl->devname, VCI_PSEUDO_UNMONITORED_DEVICE) != 0
			 && !vci_IsLowIOLoad(dl)))
		{
			if (dl->memory_entry_pos == NULL)
				elog(LOG, "wos->ros translation: skip translation on device [%s] because no memory entry", dl->devname);
			else if (!vci_IsLowIOLoad(dl))
				elog(LOG, "wos->ros translation: skip translation on device [%s] because high IO-load", dl->devname);
